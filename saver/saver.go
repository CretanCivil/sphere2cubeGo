...
func SaveTileFile(tileFile ,tileName, outPutDir string) error {
	...
	src512 := imaging.Resize(tileImage, 512, 0, imaging.Linear)
	err = imaging.Save(src512, filepath.Join(outPutDir, "tiles/512_face"+tileName+"_0_0.jpg"))

	src51k := imaging.Resize(tileImage, 1024, 0, imaging.Linear)
	for  i := 0; i < 2;i++ {
		for  j := 0; j < 2;j++ {
			src512 = imaging.Crop(src51k,image.Rectangle{Min:image.Point{i*512,j*512},Max:image.Point{(i+1)*512,(j+1)*512}})
			err = imaging.Save(src512, filepath.Join(outPutDir, fmt.Sprintf("tiles/1k_face"+tileName+"_%d_%d.jpg",i,j)))
		}
	}

	src2k := imaging.Resize(tileImage, 2048, 0, imaging.Linear)
	for  i := 0; i < 4;i++ {
		for  j := 0; j < 4;j++ {
			src512 = imaging.Crop(src2k,image.Rectangle{Min:image.Point{i*512,j*512},Max:image.Point{(i+1)*512,(j+1)*512}})
			err = imaging.Save(src512, filepath.Join(outPutDir, fmt.Sprintf("tiles/2k_face"+tileName+"_%d_%d.jpg",i,j)))
		}
	}

	src4k := imaging.Resize(tileImage, 4096, 0, imaging.Linear)
	for  i := 0; i < 8;i++ {
		for  j := 0; j < 8;j++ {
			src512 = imaging.Crop(src4k,image.Rectangle{Min:image.Point{i*512,j*512},Max:image.Point{(i+1)*512,(j+1)*512}})
			err = imaging.Save(src512, filepath.Join(outPutDir, fmt.Sprintf("tiles/4k_face"+tileName+"_%d_%d.jpg",i,j)))
		}
	}
